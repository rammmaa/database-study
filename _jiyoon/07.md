...
* 작동 원리
  1. 해시 함수를 통해 특정 slot(page)를 찾아감
  2-1. 페이지에 빈 공간이 있으면 데이터 삽입
  2-2. 페이지가 꽉 찼다면 새로운 페이지(overflow page)를 할당받고 원래 페이지에 **linked list**처럼 연결
* e.g., Java HashMap
* pros:
  * simple, easy
* cons:
...
* 작동 원리
  1. 버킷 A가 가득 참
  2-1. `if` 버킷 A의 local depth < global depth `->` 버킷 A를 2개로 나누고 디렉토리의 포인터만 수정
  2-2. `if` 버킷 A의 local depth >= global depth `->` **디렉토리 크기를 2배로 증가(doubling)**
